...
UnpackedGaussian PackedGaussian::unpack(
  ...
  if (usesFloat16) {
    ...
    float sscale = 1.0 / (1 << fractionalBits);
    for (size_t i = 0; i < 3; i++) {
      ...
      result.position[i] = c.flipP[i] * static_cast<float>(fixed32) * sscale;
    }
  }
...
PackedGaussian PackedGaussians::at(int32_t i) const {
  ...
  const auto *ssh = &this->sh[i * shDim * 3];
  for (int32_t j = 0; j < shDim; ++j, ssh += 3) {
    result.shR[j] = ssh[0];
    result.shG[j] = ssh[1];
    result.shB[j] = ssh[2];
  }
